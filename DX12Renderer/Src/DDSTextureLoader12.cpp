...
namespace
{
        ...
        D3D12_RESOURCE_DESC& resourceDesc)
    {
        ...
        resourceDesc.DepthOrArraySize = (resDim == D3D12_RESOURCE_DIMENSION_TEXTURE3D) ? static_cast<UINT16>(depth) : static_cast<UINT16>(arraySize);
        resourceDesc.Format = format;
        resourceDesc.Flags = resFlags;
        resourceDesc.SampleDesc.Count = 1;
        resourceDesc.SampleDesc.Quality = 0;
        resourceDesc.Dimension = resDim;
    ...
    HRESULT CreateTextureFromDDS(_In_ ID3D12Device* d3dDevice,
       ...
       // _Outptr_ ID3D12Resource** texture,
        ...
        D3D12_RESOURCE_DESC& resourceDesc) noexcept(false)
    {
        HRESULT hr = S_OK;
        ...
        if (SUCCEEDED(hr))
        {
            ...
            GetTextureResourceDesc(d3dDevice, resDim, twidth, theight, tdepth, reservedMips - skipMip, arraySize,
                format, resFlags, loadFlags,resourceDesc);
...
HRESULT DirectX::LoadDDSTextureFromFile(
    ...
    std::unique_ptr<uint8_t[]>& ddsData,
    std::vector<D3D12_SUBRESOURCE_DATA>& subresources,
    D3D12_RESOURCE_DESC& resourceDescOut,
    size_t maxsize,
    DDS_ALPHA_MODE* alphaMode,
    bool* isCubeMap)
{
    ...
    bool* isCubeMap)
{
    ...
    hr = CreateTextureFromDDS(d3dDevice,
        header, bitData, bitSize, maxsize,
        resFlags, loadFlags,
        subresources, isCubeMap, resourceDescOut);
