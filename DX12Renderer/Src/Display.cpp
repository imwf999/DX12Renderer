...
namespace rdr
{
		...
		: pIDXGIFactory5(pDxgi), pd3dDevice(pDev), pAdapter(pAda)
	{
		...
		swapChainDesc.BufferCount = global_SwapChainBufferCount;				//交换链中包含的后台缓冲区个数
		...
		CHECK_RESULT(pIDXGIFactory5->CreateSwapChainForHwnd(
		...
		CHECK_RESULT(pSwapChain1.As(&pSwapChain3));
		currentBackBufferIndex = pSwapChain3->GetCurrentBackBufferIndex();		//获取当前的后台缓冲区序号，SwapChain3中扩展的方法

		//创建RTV描述符
		for (int i = 0; i < global_SwapChainBufferCount; ++i)
		{
			pSwapChainBufferArr[i] = std::make_unique<RenderTexture>("BackBuffer" + std::to_string(i));
			CHECK_RESULT(pSwapChain3->GetBuffer(i, IID_PPV_ARGS(pSwapChainBufferArr[i]->GetDefaultResourceAddressOf())));
	...
	void Display::Present(const Renderer& renderer)
	{
		...
		CHECK_RESULT(pList->Close());

		ID3D12CommandList* cmdList2[] = { pList };
		pQueue->ExecuteCommandLists(_countof(cmdList2), cmdList2);

		renderer.GetUIRenderer()->DrawUI(renderer);

		CHECK_RESULT(pSwapChain3->Present(0, 0));
